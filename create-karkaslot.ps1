...
Bu repository, Windows üzerinde tek bir PowerShell betiğiyle başlatılabilen bir başlangıç uygulaması üretir.
...
- Rol bazlı erişim (Admin, Warehouse, Production, Quality, Accounting)
...
- JWT auth ve rol yapısını ekler
    ...
    public async Task<ActionResult<IEnumerable<object>>> Get()
    {
        var result = await _context.Lots
            ...
            .Select(x => new
            {
                ...
                x.ExpiryDate,
                x.ReceivedAt,
                AvailableQty = _context.StockMovements.Where(m => m.LotId == x.Id).Sum(m => m.MovementType == "IN" ? m.Qty : -m.Qty)
            })
            .ToListAsync();
    ...
    public async Task<ActionResult<IEnumerable<object>>> Get()
    {
        return await _context.Shipments
            ...
            .Select(x => new
            {
                ...
                x.CustomerName,
                x.CreatedAt,
                LotSummary = string.Join(" | ", x.Items.Select(i => i.Lot!.LotNo + ": " + i.Qty + " kg"))
            })
            .ToListAsync();
    }
...
export async function getProducts() {
  return request("/products", { headers: { Authorization: `Bearer ${localStorage.getItem("token")}` } });
}

export async function getSuppliers() {
  return request("/suppliers", { headers: { Authorization: `Bearer ${localStorage.getItem("token")}` } });
}

export async function getLots() {
  return request("/lots", { headers: { Authorization: `Bearer ${localStorage.getItem("token")}` } });
}

export async function getShipments() {
  return request("/shipments", { headers: { Authorization: `Bearer ${localStorage.getItem("token")}` } });
}

export async function getUsers() {
  return request("/users", { headers: { Authorization: `Bearer ${localStorage.getItem("token")}` } });
}

export async function getRoles() {
  return request("/roles", { headers: { Authorization: `Bearer ${localStorage.getItem("token")}` } });
}
...
import { login, getProducts, getSuppliers, getLots, getShipments, getUsers, getRoles, assignUserRole, createReceipt, createShipment } from "./services/api";
import "./index.css";
...
function App() {
  ...
  const loadDashboard = async () => {
    ...
    try {
      const [p, s, l, sh] = await Promise.all([getProducts(), getSuppliers(), getLots(), getShipments()]);
      setProducts(p);
      setSuppliers(s);
      setLots(l);
      setShipments(sh);
  ...
  const submitReceipt = async (e) => {
    ...
    try {
      ...
      setReceipt({ lotNo: "", productId: products[0]?.id || "", supplierId: suppliers[0]?.id || "", amount: "", unit: "kg", expiryDate: "", sourceRef: "" });
      loadDashboard();
    } catch (err) {
          ...
          <tbody>
            {lots.map(lot => (
              <tr key={lot.id}>
                ...
                <td>{lot.availableQty} {lot.unit}</td>
                <td>{lot.status}</td>
              </tr>
            ))}
          </tbody>
